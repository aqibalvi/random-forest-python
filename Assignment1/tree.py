...
#         print("I am C===============", c)
        ...
        purity = np.amax(c/float(len(Y)))
        
        if (nexamples<=self.exthreshold or purity>=self.purity or depth >= self.maxdepth):
            cmax = klasses[np.argmax(c)]
            leaf = Node (self.purity,klasslabel = cmax)
            return leaf
        ...
        Dy = [] 
        Dn = []
        feat,split,entropy,Dy,Dn = self.getWeakLearner().train(X,Y)
        Dy = np.array(Dy)
        Dn = np.array(Dn)
        
        
        
        if (np.sum(Dy) <= 0 or np.sum(Dn) <= 0):
            ...
            cur = Node (purity,score = entropy,wlearner = self.weaklearner )
            cur.parameters = split
            cur.feat = feat
            lchild=self.build_tree (X[Dy,:],Y[Dy],depth+1)
            rchild = self.build_tree (X[Dn,:],Y[Dn],depth+1)    
            if lchild==None or rchild==None:
                print ('problem child')
                print (Dy)
                print (Dn)
       
            cur.set_childs (lchild,rchild)
...
#         print ( "I am XXXXXX======", X.shape)
    ...
    def predict(self, X):
        ...
        for iterX in range(X.shape[0]):
            ...
            z.append(self._predict(self.tree,np.atleast_2d(X[iterX,:])))
    ...
    def _predict(self,node, X):
        ...
        if (self.weaklearner == "Axis-Aligned"):
            boolValue = self.getWeakLearner().evaluate(node.parameters,node.feat,X[0])
        else: 
            boolValue = self.getWeakLearner().evaluate(node.parameters,node.feat[0],node.feat[1],X[0])
